...
支持多窗口编辑，每个窗口独立管理文档。通过工具栏按钮或 `Ctrl+Shift+N` 创建空窗口，通过文件目录点击文件直接在新窗口打开。
...
| 📁 | 文件浏览 | — | 展开左侧文件列表面板，列出当前目录所有 .md/.txt 文件 |
...
标题栏 📁 按钮展开左侧文件列表面板，自动列出当前编辑文件所在目录的全部 `.md` 和 `.txt` 文件：

- 点击 📁 展开/收起面板（覆盖层设计，不挤压编辑区）
- 文件按字母排序，当前打开的文件高亮显示
- **普通点击文件名 → 在新窗口打开文件**（无保存确认，不干扰当前窗口）
- **Shift+点击文件名 → 在当前编辑区打开**（如有未保存变更会弹确认框）
- 翻页浏览：每页显示 18 个文件，顶部 `▲ Prev` / 底部 `▼ Next` 翻页
- 多页时底部显示页码指示（如 `2/5`）
- 点击面板外区域关闭面板
- 空目录显示「No .md or .txt files found」提示
...
- **文件浏览器** — 标题栏 📁 按钮展开左侧面板，浏览并快速切换当前目录的 .md/.txt 文件
- **光标列记忆** — 上下移动光标时记忆首选视觉列，跨越短行/空行后自动恢复到正确列位
